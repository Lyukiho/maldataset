...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Treat rows whose source starts with this prefix as the test set, "
            "and every other row as the training set. When omitted, use the "
            "original per-label random train/test split."
        ),
    )
...
def main() -> int:
    ...
    if args.source_test_prefix is not None:
        prefix = args.source_test_prefix.casefold()
        train_rows = [r for r in rows if not r.get("source", "").casefold().startswith(prefix)]
        test_rows = [r for r in rows if r.get("source", "").casefold().startswith(prefix)]
        print(f"  Source holdout: test=source starting with '{args.source_test_prefix}'")
    else:
